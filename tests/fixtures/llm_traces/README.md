...
|----------|----------|---------|-------------|
| `T3CLAW_RECORD_TRACE` | yes | — | Set to any non-empty value to enable recording. |
| `T3CLAW_TRACE_OUTPUT` | no | `./trace_{timestamp}.json` | Output file path for the recorded trace. |
| `T3CLAW_TRACE_MODEL_NAME` | no | `recorded-{model}` | The `model_name` field in the trace JSON. |

### Usage

```bash
# Record a trace (writes to ./trace_20260304T120000.json)
T3CLAW_RECORD_TRACE=1 cargo run

# Custom output path
T3CLAW_RECORD_TRACE=1 T3CLAW_TRACE_OUTPUT=my_trace.json cargo run

# Custom model name
T3CLAW_RECORD_TRACE=1 T3CLAW_TRACE_MODEL_NAME=regression-auth-flow cargo run
```
