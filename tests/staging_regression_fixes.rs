...
//!    `/opt/t3claw/config/worker/mcp-servers.json`, but the real config
//!    lives at `~/.t3claw/mcp-servers.json` and bootstrap migrates that
...
///
/// Note: `load_mcp_servers_from_db` falls back to `~/.t3claw/mcp-servers.json`
