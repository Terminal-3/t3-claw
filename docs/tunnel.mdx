...
A tunnel exposes your local T3Claw agent to the internet. You need it for webhook-based channels and for instant message delivery where polling is not desired.
...
`ngrok` is a managed tunnel service with a minimal setup, ideal if you are just starting with `t3claw`. To use it, you will need to get an auth token from the [ngrok dashboard](https://dashboard.ngrok.com/get-started/your-authtoken)
...
- `{port}` for T3Claw's local port
- `{host}` for T3Claw's local host
...
Use this option when the tunnel is managed outside T3Claw and you already have a stable public URL.

T3Claw will use that URL directly and will not start or manage any tunnel process.
