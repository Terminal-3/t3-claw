...
description: "Build a WASM messaging channel that plugs into BastionClaw"
---
...
version = "0.1.0"
edition = "2021"
description = "My messaging platform channel for BastionClaw"
...
mkdir -p ~/.bastionclaw/channels
cp my-channel.wasm ~/.bastionclaw/channels/my-channel.wasm
cp my-channel.capabilities.json ~/.bastionclaw/channels/
```
...
mkdir -p ~/.bastionclaw/channels
cp channels-src/telegram/telegram.wasm channels-src/telegram/telegram.capabilities.json ~/.bastionclaw/channels/
```

<Danger>
If you are contributing a channel to the public repository, **do not commit compiled WASM binaries.** They are a supply chain risk — the binary in a PR may not match the source. BastionClaw builds channels from source.
</Danger>
