...
description: "Build a WASM messaging channel that plugs into T3Claw"
---
...
version = "0.1.0"
edition = "2021"
description = "My messaging platform channel for T3Claw"
...
mkdir -p ~/.t3claw/channels
cp my-channel.wasm ~/.t3claw/channels/my-channel.wasm
cp my-channel.capabilities.json ~/.t3claw/channels/
```
...
mkdir -p ~/.t3claw/channels
cp channels-src/telegram/telegram.wasm channels-src/telegram/telegram.capabilities.json ~/.t3claw/channels/
```

<Danger>
If you are contributing a channel to the public repository, **do not commit compiled WASM binaries.** They are a supply chain risk — the binary in a PR may not match the source. T3Claw builds channels from source.
</Danger>
