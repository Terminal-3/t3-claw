...
The HTTP Webhook channel provides a REST API for integrating external services with BastionClaw.
...
Configure GitHub to send events to your BastionClaw webhook URL:
...
Use Zapier's Webhook action to send events to BastionClaw.
...
<AccordionGroup>
  ...
  <Accordion title="Messages not processed" icon="message-circle">
    - Check logs: `RUST_LOG=bastionclaw=debug bastionclaw run`
    - Verify JSON format
