...
The HTTP Webhook channel provides a REST API for integrating external services with T3Claw.
...
Configure GitHub to send events to your T3Claw webhook URL:
...
Use Zapier's Webhook action to send events to T3Claw.
...
<AccordionGroup>
  ...
  <Accordion title="Messages not processed" icon="message-circle">
    - Check logs: `RUST_LOG=t3claw=debug t3claw run`
    - Verify JSON format
