...
WASM tools run inside a WebAssembly sandbox powered by [wasmtime](https://wasmtime.dev/). They have access to BastionClaw's host functions (logging, time, workspace), but all other capabilities — network, filesystem, credentials — must be explicitly declared in a `capabilities.json` file.

This is BastionClaw's recommended approach for custom integrations that need stronger isolation than a built-in Rust tool provides.
...
LLM selects tool
    ↓
BastionClaw loads WASM module (from cache or disk)
    ↓
...
WASM modules can call the following host functions provided by BastionClaw:

| Function | Description |
|----------|-------------|
| `log(level, message)` | Write to the BastionClaw log at the given severity level |
...
- `~/.bastionclaw/tools/` — User-installed tools
...
# Install from a URL
bastionclaw tool install https://example.com/tools/my-tool.wasm
...
The cache is stored at `~/.bastionclaw/wasm-cache/`.
