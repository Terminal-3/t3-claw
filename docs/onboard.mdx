...
<Danger>
If you are new to BastionClaw, we recommend you to configure [channels](/channels/telegram), tools, and other settings one at a time instead of 
...
If you are new to BastionClaw, we recommend you to configure channels, tools, and other settings one at a time instead of all at once through the onboard command.
...
The wizard will first ask you to select a path for the agent's database, by default `/home/agent/.bastionclaw/bastionclaw.db`. This is where the agent will store your configuration.
...
BastionClaw can execute code, run builds, and use tools inside Docker
...
If you are running BastionClaw in an environment without Docker (like a server or a container), you can disable sandboxing
</Info>
