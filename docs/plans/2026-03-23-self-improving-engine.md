...
**What it changes:** `crates/t3claw_engine/prompts/*.md` files
...
**Validation:** Run existing test suite (`cargo test -p t3claw_engine`). Only apply if tests pass.

### Level 3: Code Patching (high risk, high value)

The engine proposes Rust code changes to fix bugs it detects in itself.

**What it changes:** Any file in `crates/t3claw_engine/` or `src/bridge/`
...
2. Run full test suite (`cargo test -p t3claw_engine`)
3. Run clippy (`cargo clippy -p t3claw_engine --all-targets -- -D warnings`)
4. Never modify files outside `crates/t3claw_engine/` and `src/bridge/` without human approval
...
```
You are a debugging agent analyzing execution traces from the T3Claw engine.
...
- Prompt templates: crates/t3claw_engine/prompts/*.md
- Bridge adapters: src/bridge/*.rs
- Engine code: crates/t3claw_engine/src/**/*.rs
...
- Always run `cargo test -p t3claw_engine` after making changes
...
4. Runs `cargo test -p t3claw_engine` and `cargo clippy`
