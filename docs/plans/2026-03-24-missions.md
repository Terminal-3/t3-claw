...
Already defined in `crates/bastionclaw_engine/src/types/mission.rs`.
...
`crates/bastionclaw_engine/src/runtime/mission.rs` — already has types, needs execution logic:
...
`crates/bastionclaw_engine/src/runtime/mission_prompt.rs`:
...
```python
mission_create(
    goal="Increase BastionClaw test coverage from 60% to 80%",
