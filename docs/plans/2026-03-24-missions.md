...
Already defined in `crates/t3claw_engine/src/types/mission.rs`.
...
`crates/t3claw_engine/src/runtime/mission.rs` — already has types, needs execution logic:
...
`crates/t3claw_engine/src/runtime/mission_prompt.rs`:
...
```python
mission_create(
    goal="Increase T3Claw test coverage from 60% to 80%",
