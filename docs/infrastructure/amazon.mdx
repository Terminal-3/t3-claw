---
title: Amazon EC2
description: Host BastionClaw on an Amazon EC2 Instance
---

Amazon Elastic Compute Cloud (EC2) lets you run virtual machines on AWS infrastructure with flexible pricing and a free tier for new accounts. In this guide we will launch an EC2 instance and configure it securely so you can run BastionClaw and expose it to the internet.

<Tip>
Do not feel like setting up your own infrastructure? You can install BastionClaw with a few clicks on [agent.near.ai](https://agent.near.ai)
</Tip>
...
- **Name**: choose a descriptive name (e.g. `bastionclaw`)
...
Now that we are inside the instance, we need to perform some initial configuration. We want to strengthen security by setting up a dedicated user and preparing the system for running BastionClaw.
...
It is good practice to create a dedicated user with sudo privileges instead of using `ubuntu` for all operations. You can create a new user (for example, `bastionclaw`) and add it to the sudo group:

```bash
sudo adduser bastionclaw
sudo usermod -aG sudo bastionclaw
```
...
sudo mkdir -p /home/bastionclaw/.ssh

# Copy the authorized_keys from the ubuntu user
sudo cp ~/.ssh/authorized_keys /home/bastionclaw/.ssh/authorized_keys

# Set the correct ownership (critical — SSH will ignore the file otherwise)
sudo chown -R bastionclaw:bastionclaw /home/bastionclaw/
sudo chmod 700 /home/bastionclaw/.ssh
sudo chmod 600 /home/bastionclaw/.ssh/authorized_keys
```

Open a new terminal window and confirm you can log in with the new user before continuing:

```bash
ssh -i <KEY_PATH> bastionclaw@<IP_ADDRESS>
```
...
Now that the instance is set up and secured, install BastionClaw:

```bash
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/nearai/bastionclaw/releases/latest/download/bastionclaw-installer.sh | sh
```

Then start BastionClaw and follow the instructions to complete the setup:
...
<Tip>
We recommend using a session manager like `tmux` or `screen` so you can easily detach and reattach to your running BastionClaw instance between SSH sessions.
</Tip>
...
Follow our [Quickstart Guide](/quickstart) to create your first agent, connect it to Telegram, and start exploring BastionClaw's capabilities.
