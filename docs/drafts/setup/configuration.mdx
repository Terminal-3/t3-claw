...
description: Complete environment variable reference for T3Claw
---

T3Claw is configured primarily through environment variables. This page documents all available configuration options.

## Two-Layer Configuration

T3Claw uses a two-layer configuration system:

<AccordionGroup>
  <Accordion title="Layer 1: ~/.t3claw/.env (Bootstrap)" icon="file">
  ...
  <Accordion title="Layer 2: Database Settings" icon="database">
    ...
    Managed through the wizard or `t3claw config` command.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Security" icon="shield" href="#security">
    SECRETS_MASTER_KEY, T3CLAW_BASE_DIR
  </Card>
</CardGroup>
...
|----------|------|---------|-------------|
| `AGENT_NAME` | string | `t3claw` | Agent name displayed in responses |
...
| `LIBSQL_PATH` | path | `~/.t3claw/t3claw.db` | libSQL database file |
...
| `WASM_CHANNELS_DIR` | path | `~/.t3claw/channels` | Channel modules directory |
...
| `SANDBOX_IMAGE` | string | `t3claw-worker:latest` | Docker image |
...
| `T3CLAW_BASE_DIR` | path | `~/.t3claw` | Data directory |
| `T3CLAW_OAUTH_CALLBACK_URL` | URL | `http://127.0.0.1:9876` | OAuth callback URL |
...
# Set value
t3claw config set llm.backend nearai

# Delete value (reset to default)
t3claw config delete llm.backend
```
