...
description: Complete environment variable reference for BastionClaw
---

BastionClaw is configured primarily through environment variables. This page documents all available configuration options.

## Two-Layer Configuration

BastionClaw uses a two-layer configuration system:

<AccordionGroup>
  <Accordion title="Layer 1: ~/.bastionclaw/.env (Bootstrap)" icon="file">
  ...
  <Accordion title="Layer 2: Database Settings" icon="database">
    ...
    Managed through the wizard or `bastionclaw config` command.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Security" icon="shield" href="#security">
    SECRETS_MASTER_KEY, BASTIONCLAW_BASE_DIR
  </Card>
</CardGroup>
...
|----------|------|---------|-------------|
| `AGENT_NAME` | string | `bastionclaw` | Agent name displayed in responses |
...
| `LIBSQL_PATH` | path | `~/.bastionclaw/bastionclaw.db` | libSQL database file |
...
| `WASM_CHANNELS_DIR` | path | `~/.bastionclaw/channels` | Channel modules directory |
...
| `SANDBOX_IMAGE` | string | `bastionclaw-worker:latest` | Docker image |
...
| `BASTIONCLAW_BASE_DIR` | path | `~/.bastionclaw` | Data directory |
| `BASTIONCLAW_OAUTH_CALLBACK_URL` | URL | `http://127.0.0.1:9876` | OAuth callback URL |

## Environment File Example

Create `~/.bastionclaw/.env`:
...
# Set value
bastionclaw config set llm.backend nearai

# Delete value (reset to default)
bastionclaw config delete llm.backend
```
