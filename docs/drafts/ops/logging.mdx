...
BastionClaw uses the [tracing](https://docs.rs/tracing) crate for structured logging. Output goes to stdout/stderr and can be streamed in real time via the Web Gateway SSE endpoint.
...
|---------------|----------|
| `bastionclaw=error` | Production: errors only |
| `bastionclaw=warn` | Production: errors and warnings |
| `bastionclaw=info` | Production: normal operational events (recommended) |
| `bastionclaw=debug` | Troubleshooting: detailed request/response flow |
| `bastionclaw=trace` | Deep debugging: all internal events including LLM token streams |
| `bastionclaw=info,tower_http=warn` | Reduce HTTP access log noise |
| `bastionclaw=debug,tower_http=debug` | Debug with HTTP request details |

Set in your environment file or shell:

```bash
# In .env or /etc/bastionclaw/bastionclaw.env
RUST_LOG=bastionclaw=info,tower_http=warn
...
RUST_LOG=bastionclaw::agent=debug,bastionclaw::agent::scheduler=trace

# LLM provider calls only
RUST_LOG=bastionclaw::llm=debug

# Safety layer only
RUST_LOG=bastionclaw::safety=debug

# Skills system only
RUST_LOG=bastionclaw::skills=debug

# Sandbox and proxy
RUST_LOG=bastionclaw::sandbox=debug

# Database queries
RUST_LOG=bastionclaw::db=trace

# Everything + HTTP access log
RUST_LOG=bastionclaw=debug,tower_http=debug

# Silence noisy dependencies
RUST_LOG=bastionclaw=info,hyper=warn,reqwest=warn,tower_http=warn
```
...
BastionClaw emits structured logs in the following format:

```
2024-01-15T10:30:00.123456Z  INFO bastionclaw::agent::worker: job started job_id="job_01j9abc" intent="summarize recent activity" session="default"
2024-01-15T10:30:00.456789Z DEBUG bastionclaw::llm::nearai_chat: sending request model="claude-3-5-sonnet-20241022" tokens=1024
2024-01-15T10:30:02.891234Z  INFO bastionclaw::agent::worker: tool call tool="memory_search" job_id="job_01j9abc"
2024-01-15T10:30:03.234567Z  INFO bastionclaw::agent::worker: job completed job_id="job_01j9abc" duration_ms=3111 tokens_used=1847
```
...
| `module` | Rust module path (e.g., `bastionclaw::agent::worker`) |
...
# Last 200 lines
journalctl -u bastionclaw -n 200

# Since a specific time
journalctl -u bastionclaw --since "2024-01-15 10:00:00"

# Last hour only
journalctl -u bastionclaw --since "1 hour ago"

# Last hour, error level and above
journalctl -u bastionclaw --since "1 hour ago" -p err

# Filter by a specific string (grep equivalent)
journalctl -u bastionclaw -g "job_01j9abc"

# Export to JSON for analysis
journalctl -u bastionclaw --since today -o json > bastionclaw-today.json
...
BastionClaw records every LLM API call in the `llm_calls` database table. Each record includes:
...
sqlite3 ~/.bastionclaw/bastionclaw.db
...
```
event: log
data: {"level":"INFO","message":"job started","module":"bastionclaw::agent::worker","job_id":"job_01j9abc","ts":"2024-01-15T10:30:00Z"}
...
Some dependencies are verbose at the default log level. Silence them while keeping BastionClaw output at debug:

```bash
# Quiet HTTP infrastructure
RUST_LOG=bastionclaw=debug,hyper=warn,reqwest=warn,tower_http=warn,h2=warn

# Quiet database layer
RUST_LOG=bastionclaw=info,bastionclaw::db=warn

# Maximum quiet (errors only everywhere)
RUST_LOG=error

# Recommended production setting
RUST_LOG=bastionclaw=info,tower_http=warn,hyper=warn
```
...
RUST_LOG=bastionclaw=debug,bastionclaw::sandbox=off

# Suppress all LLM call details
RUST_LOG=bastionclaw=debug,bastionclaw::llm=info

# Suppress routine engine tick logs
RUST_LOG=bastionclaw=info,bastionclaw::agent::routine_engine=warn
```
