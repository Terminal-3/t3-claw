...
T3Claw uses two sandbox layers for tool execution: WASM sandbox for tools, and Docker sandbox for jobs.
...
# Docker image
export SANDBOX_IMAGE=t3claw-worker:latest
```
...
T3Claw can run inside Docker and still sandbox jobs:
...
<Warning>
**T3Claw runs alongside Docker** (for job sandboxing), not inside Docker by default.

- **Default**: T3Claw binary → spawns containers for jobs
- **Optional**: T3Claw inside container → still spawns sibling containers
</Warning>

See [Docker Install](/install/docker) for running T3Claw itself in a container.
