...
BastionClaw uses two sandbox layers for tool execution: WASM sandbox for tools, and Docker sandbox for jobs.
...
# Docker image
export SANDBOX_IMAGE=bastionclaw-worker:latest
```
...
BastionClaw can run inside Docker and still sandbox jobs:
...
<Warning>
**BastionClaw runs alongside Docker** (for job sandboxing), not inside Docker by default.

- **Default**: BastionClaw binary → spawns containers for jobs
- **Optional**: BastionClaw inside container → still spawns sibling containers
</Warning>

See [Docker Install](/install/docker) for running BastionClaw itself in a container.
