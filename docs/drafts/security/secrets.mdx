...
BastionClaw uses a zero-exposure credential model: secrets are encrypted at rest and never exposed to untrusted code.
...
```bash
# List secrets
bastionclaw secret list

# Get a secret (decrypted)
bastionclaw secret get telegram_bot_token

# Set a secret
bastionclaw secret set telegram_bot_token "your-token"

# Delete a secret
bastionclaw secret delete telegram_bot_token
```
...
Common secret names used by BastionClaw:
...
<AccordionGroup>
  ...
  <Accordion title="Secret not found" icon="search">
    ...
    - Re-run `bastionclaw onboard` to reconfigure
  </Accordion>
