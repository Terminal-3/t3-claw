...
Common questions about BastionClaw.

## General

<AccordionGroup>
  <Accordion title="What is BastionClaw?" icon="help-circle">
    BastionClaw is a secure, self-hosted AI assistant that runs on your own hardware. It provides a personal AI with strong privacy guarantees through multi-layer security.
  </Accordion>

  <Accordion title="Do I need Docker?" icon="container">
    **No for basic use, yes for job sandboxing.**

    BastionClaw runs as a standalone binary. Docker is only required if you want to use:
    - Sandboxed job execution (Claude Code mode)
    - WASM tool building with automatic compilation

    Without Docker, BastionClaw still works fully — just without the Docker sandbox layer.
  </Accordion>

  <Accordion title="Can I run without PostgreSQL?" icon="database">
    **Yes.** BastionClaw includes libSQL (embedded SQLite) as an alternative.
  ...
  <Accordion title="How is BastionClaw different from openclaw?" icon="git-compare">
    | Feature | BastionClaw | openclaw |
    |---------|----------|----------|
    ...
    BastionClaw is the self-hosted version for users who want complete control over their data.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Where is my data stored?" icon="database">
    By default: `~/.bastionclaw/`

    ```
    ~/.bastionclaw/
    ├── bastionclaw.db          # libSQL database
    ...
    Change with `BASTIONCLAW_BASE_DIR`.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Why is the wizard asking for a browser?" icon="globe">
    ...
    - Or set `BASTIONCLAW_OAUTH_CALLBACK_URL` to a public URL
  </Accordion>

  <Accordion title="How do I run BastionClaw as a service?" icon="server">
    ```bash
    # Install service
    bastionclaw service install

    # Start
    sudo systemctl enable --now bastionclaw  # Linux
    brew services start bastionclaw            # macOS
    ```
  </Accordion>

  <Accordion title="Can I use BastionClaw behind a reverse proxy?" icon="network">
...
<AccordionGroup>
  <Accordion title="How secure is BastionClaw?" icon="shield">
    BastionClaw uses defense in depth:
  ...
  <Accordion title="Can BastionClaw be hacked?" icon="alert-triangle">
    No software is unhackable. BastionClaw reduces risk through:
...
<AccordionGroup>
  <Accordion title="Can I run BastionClaw on a Raspberry Pi?" icon="cpu">
    **Yes**, with caveats:
  ...
  <Accordion title="Can I contribute?" icon="github">
    Yes! BastionClaw is open source:

    - GitHub: https://github.com/bastionclaw-ai/bastionclaw
...
- [GitHub Issues](https://github.com/bastionclaw-ai/bastionclaw/issues) — Bug reports
