...
Solutions for common BastionClaw issues.
...
<AccordionGroup>
  ...
  <Accordion title="Permission denied" icon="lock">
    **Solution:**
    ...
    sudo mv ~/.local/bin/bastionclaw /usr/local/bin/
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Database connection failed" icon="database">
    **PostgreSQL:**
    ...
    # Verify connection
    psql postgres://user:pass@localhost/bastionclaw
    ```
...
<AccordionGroup>
  <Accordion title="Browser didn't open for OAuth" icon="globe">
    **Solutions:**
    ...
    export BASTIONCLAW_OAUTH_CALLBACK_URL=https://your-server:9876
    ```
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Web Gateway token not found" icon="key">
    **Solution:**
    ```bash
    # View logs
    RUST_LOG=bastionclaw=info bastionclaw run 2>&1 | grep "Gateway auth token"
  ...
  <Accordion title="Owner binding timeout" icon="clock">
    **Solutions:**
    ...
    2. Re-run `bastionclaw onboard --channels-only`
...
```bash
# All modules
RUST_LOG=debug bastionclaw run

# Just BastionClaw
RUST_LOG=bastionclaw=debug bastionclaw run

# Specific module
RUST_LOG=bastionclaw::agent=debug bastionclaw run

# With HTTP requests
RUST_LOG=bastionclaw=debug,tower_http=debug bastionclaw run
```
...
1. Run `bastionclaw doctor --json`
2. Check logs with `RUST_LOG=debug`
3. Search [GitHub Issues](https://github.com/bastionclaw-ai/bastionclaw/issues)
