...
Solutions for common T3Claw issues.
...
<AccordionGroup>
  ...
  <Accordion title="Permission denied" icon="lock">
    **Solution:**
    ...
    sudo mv ~/.local/bin/t3claw /usr/local/bin/
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Database connection failed" icon="database">
    **PostgreSQL:**
    ...
    # Verify connection
    psql postgres://user:pass@localhost/t3claw
    ```
...
<AccordionGroup>
  <Accordion title="Browser didn't open for OAuth" icon="globe">
    **Solutions:**
    ...
    export T3CLAW_OAUTH_CALLBACK_URL=https://your-server:9876
    ```
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Web Gateway token not found" icon="key">
    **Solution:**
    ```bash
    # View logs
    RUST_LOG=t3claw=info t3claw run 2>&1 | grep "Gateway auth token"
  ...
  <Accordion title="Owner binding timeout" icon="clock">
    **Solutions:**
    ...
    2. Re-run `t3claw onboard --channels-only`
...
```bash
# All modules
RUST_LOG=debug t3claw run

# Just T3Claw
RUST_LOG=t3claw=debug t3claw run

# Specific module
RUST_LOG=t3claw::agent=debug t3claw run

# With HTTP requests
RUST_LOG=t3claw=debug,tower_http=debug t3claw run
```
...
1. Run `t3claw doctor --json`
2. Check logs with `RUST_LOG=debug`
3. Search [GitHub Issues](https://github.com/t3claw-ai/t3claw/issues)
