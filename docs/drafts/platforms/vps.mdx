...
description: Securing T3Claw on a VPS with UFW, Caddy, and fail2ban
---

Deploying T3Claw on a public VPS requires additional hardening. Never expose the Web Gateway port directly to the internet — route all external traffic through a TLS-terminating reverse proxy and restrict direct port access with a firewall.

---

## Create a Dedicated User

Run T3Claw as a non-root user with Docker access:

```bash
# Create user
sudo adduser --disabled-password --gecos "" t3claw

# Add to docker group (for sandbox)
sudo usermod -aG docker t3claw

# Create config directory
sudo mkdir -p /etc/t3claw
sudo chown root:t3claw /etc/t3claw
sudo chmod 750 /etc/t3claw

# Create data directory
sudo mkdir -p /var/lib/t3claw
sudo chown t3claw:t3claw /var/lib/t3claw
```
...
# Block direct access to T3Claw from public internet
...
Replace `t3claw.yourdomain.com` with your actual domain. Edit `/etc/caddy/Caddyfile`:

```caddy
t3claw.yourdomain.com {
  ...
  # Access log
  log {
    output file /var/log/caddy/t3claw-access.log
    format json
  }
}
```
...
curl -I https://t3claw.yourdomain.com/api/health
```
...
Create `/etc/nginx/sites-available/t3claw`:

```nginx
server {
    listen 80;
    server_name t3claw.yourdomain.com;
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/t3claw.yourdomain.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/t3claw.yourdomain.com/privkey.pem;
    ...
    access_log /var/log/nginx/t3claw.access.log;
    error_log /var/log/nginx/t3claw.error.log;
}
```
...
```bash
sudo ln -s /etc/nginx/sites-available/t3claw /etc/nginx/sites-enabled/
sudo nginx -t
sudo systemctl reload nginx
sudo certbot --nginx -d t3claw.yourdomain.com
```
...
# Create tunnel
cloudflared tunnel create t3claw

# Configure: ~/.cloudflared/config.yml
cat > ~/.cloudflared/config.yml <<'EOF'
tunnel: <tunnel-id>
credentials-file: /home/t3claw/.cloudflared/<tunnel-id>.json

ingress:
  - hostname: t3claw.yourdomain.com
    service: http://localhost:3000
  - service: http_status:404
EOF

# Route DNS
cloudflared tunnel route dns t3claw t3claw.yourdomain.com
...
port     = 443,3000
filter   = t3claw
logpath  = /var/log/caddy/t3claw-access.log
           /var/log/nginx/t3claw.access.log
maxretry = 10
bantime  = 1800
findtime = 300
action   = ufw
