...
description: Deploy BastionClaw to a cloud server
---

Deploy BastionClaw to a remote VPS or cloud server for always-on operation.
...
CREATE USER bastionclaw WITH PASSWORD 'your-secure-password';
CREATE DATABASE bastionclaw OWNER bastionclaw;
EOF
...
# Enable the vector extension in the bastionclaw database
...
2. **Custom callback URL**: Set `BASTIONCLAW_OAUTH_CALLBACK_URL` to a publicly reachable URL
...
1. **Database**: Select "PostgreSQL"
   - Enter connection string: `postgres://bastionclaw:your-secure-password@localhost/bastionclaw`
...
<Warning>
**Important:** BastionClaw's orchestrator binds to `0.0.0.0:50051` on Linux for container communication. This port should **not** be exposed externally. The firewall configuration below includes rules to block external access to this port—do not add any UFW allow rules for `50051`.
</Warning>
...
# Configure site
sudo tee /etc/nginx/sites-available/bastionclaw <<'EOF'
server {
...
# Enable site
sudo ln -sf /etc/nginx/sites-available/bastionclaw /etc/nginx/sites-enabled/
...
# Configure service
sudo systemctl edit bastionclaw
```
...
Environment="DATABASE_URL=postgres://bastionclaw:your-secure-password@localhost/bastionclaw"
...
# Create filter
sudo tee /etc/fail2ban/filter.d/bastionclaw.conf <<'EOF'
[Definition]
...
# Create jail
sudo tee /etc/fail2ban/jail.d/bastionclaw.conf <<EOF
[bastionclaw]
enabled = true
port = http,https
filter = bastionclaw
logpath = /var/log/bastionclaw/web.log
maxretry = 5
bantime = 3600
EOF
