...
description: Deploy T3Claw to a cloud server
---

Deploy T3Claw to a remote VPS or cloud server for always-on operation.
...
CREATE USER t3claw WITH PASSWORD 'your-secure-password';
CREATE DATABASE t3claw OWNER t3claw;
EOF
...
# Enable the vector extension in the t3claw database
...
2. **Custom callback URL**: Set `T3CLAW_OAUTH_CALLBACK_URL` to a publicly reachable URL
...
1. **Database**: Select "PostgreSQL"
   - Enter connection string: `postgres://t3claw:your-secure-password@localhost/t3claw`
...
<Warning>
**Important:** T3Claw's orchestrator binds to `0.0.0.0:50051` on Linux for container communication. This port should **not** be exposed externally. The firewall configuration below includes rules to block external access to this port—do not add any UFW allow rules for `50051`.
</Warning>
...
# Configure site
sudo tee /etc/nginx/sites-available/t3claw <<'EOF'
server {
...
# Enable site
sudo ln -sf /etc/nginx/sites-available/t3claw /etc/nginx/sites-enabled/
...
Environment="DATABASE_URL=postgres://t3claw:your-secure-password@localhost/t3claw"
...
# Create filter
sudo tee /etc/fail2ban/filter.d/t3claw.conf <<'EOF'
[Definition]
...
# Create jail
sudo tee /etc/fail2ban/jail.d/t3claw.conf <<EOF
[t3claw]
enabled = true
port = http,https
filter = t3claw
logpath = /var/log/t3claw/web.log
maxretry = 5
bantime = 3600
EOF
