...
description: Choose how to install BastionClaw
---

BastionClaw can run in multiple environments — from your local machine to a cloud VPS. Choose the installation method that fits your needs.

## Installation Methods

<CardGroup cols={2}>
  <Card title="Local Install" icon="monitor" href="/install/local">
    Run BastionClaw directly on your machine. Best for personal use.
  ...
  <Card title="Docker" icon="container" href="/install/docker">
    Run BastionClaw in a container. Good for consistent environments.
...
<AccordionGroup>
  ...
  <Accordion title="Optional: Docker" icon="box">
    Docker is **optional** for running BastionClaw itself, but required if you want to use:
...
- **Auto-created**: Database created automatically at `~/.bastionclaw/bastionclaw.db`
...
- [ ] Run the onboarding wizard: `bastionclaw onboard`
- [ ] Start BastionClaw: `bastionclaw run`
...
<Steps>
  <Step title="Install BastionClaw" icon="download">
  ...
  <Step title="Start Using BastionClaw" icon="message-square">
