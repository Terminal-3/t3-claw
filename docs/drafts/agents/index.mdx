...
description: How T3Claw's agent runtime processes requests and executes jobs
---

T3Claw's agent runtime is an always-on loop that accepts messages from any channel, classifies intent, and dispatches work as parallel jobs — each independently tracked through a defined state machine.
...
T3Claw uses a three-level session hierarchy:
...
As conversations grow, context pressure increases. T3Claw handles this automatically:
