...
| `metadata.bastionclaw.requires.bins` | string[] | No | Binaries that must exist on `PATH`. Skill is gated out if any are missing. |
| `metadata.bastionclaw.requires.env` | string[] | No | Environment variables that must be set. Skill is gated out if any are missing. |
...
~/.bastionclaw/skills/my-skill/SKILL.md
