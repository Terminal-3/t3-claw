...
| `metadata.t3claw.requires.bins` | string[] | No | Binaries that must exist on `PATH`. Skill is gated out if any are missing. |
| `metadata.t3claw.requires.env` | string[] | No | Environment variables that must be set. Skill is gated out if any are missing. |
...
~/.t3claw/skills/my-skill/SKILL.md
