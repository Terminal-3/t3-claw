...
description: Default LLM provider for T3Claw
---

NEAR AI is the default provider for T3Claw, offering access to multiple models including Claude via a simple OAuth flow.
...
**How it works:**
1. `t3claw onboard` opens your browser
2. Log in with GitHub or Google
3. Session token saved to `~/.t3claw/session.json`
4. Automatic renewal
...
<Warning>
**VPS / Remote servers:** Browser OAuth won't work without a browser. Use API key mode or set `T3CLAW_OAUTH_CALLBACK_URL` to a publicly reachable URL.
</Warning>
...
```bash
t3claw config set nearai.session_token sess_xxxxx
```

Or edit `~/.t3claw/session.json`:
