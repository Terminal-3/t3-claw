...
description: Default LLM provider for BastionClaw
---

NEAR AI is the default provider for BastionClaw, offering access to multiple models including Claude via a simple OAuth flow.
...
**How it works:**
1. `bastionclaw onboard` opens your browser
2. Log in with GitHub or Google
3. Session token saved to `~/.bastionclaw/session.json`
4. Automatic renewal
...
3. Key saved to `~/.bastionclaw/.env`
...
<Warning>
**VPS / Remote servers:** Browser OAuth won't work without a browser. Use API key mode or set `BASTIONCLAW_OAUTH_CALLBACK_URL` to a publicly reachable URL.
</Warning>
...
```bash
bastionclaw config set nearai.session_token sess_xxxxx
```

Or edit `~/.bastionclaw/session.json`:
