...
This guide will get you from zero to a running T3Claw instance in under 10 minutes
...
<Step title="Get T3Claw">

<Tabs>
  <Tab title="Agent Hub">
  Go to https://agent.near.ai/ and login with your preferred method, then create an T3Claw agent in a private instance.
  ...
  To use T3Claw, you will need to provide an SSH key. If you don't have one, you can generate it using the following command in your terminal:
...
<Accordion title="⚠️ Error: Another T3Claw instance is already running">

If you get the error `Error: Another T3Claw instance is already running (PID 38). If this is incorrect, remove the stale PID file: /home/agent/.t3claw/t3claw.pid`, simply run the following command to remove the stale PID file and try starting the agent again:

```
# Remove the stale PID file
rm /home/agent/.t3claw/t3claw.pid
...
<Step title="Update T3Claw">

Finally, make sure to regularly update T3Claw to get the latest features and improvements. You can update T3Claw by running the following command in your terminal:
