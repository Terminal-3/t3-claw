...
This guide will get you from zero to a running BastionClaw instance in under 10 minutes
...
<Step title="Get BastionClaw">

<Tabs>
  <Tab title="Agent Hub">
  Go to https://agent.near.ai/ and login with your preferred method, then create an BastionClaw agent in a private instance.
  ...
  To use BastionClaw, you will need to provide an SSH key. If you don't have one, you can generate it using the following command in your terminal:
...
<Accordion title="⚠️ Error: Another BastionClaw instance is already running">

If you get the error `Error: Another BastionClaw instance is already running (PID 38). If this is incorrect, remove the stale PID file: /home/agent/.bastionclaw/bastionclaw.pid`, simply run the following command to remove the stale PID file and try starting the agent again:

```
# Remove the stale PID file
rm /home/agent/.bastionclaw/bastionclaw.pid
...
<Step title="Update BastionClaw">

Finally, make sure to regularly update BastionClaw to get the latest features and improvements. You can update BastionClaw by running the following command in your terminal:
