...
//! - **Tier 3 (removed):** Database, tool count, features → use `t3claw status`.
...
pub fn print_boot_screen(info: &BootInfo) {
    ...
    // Hint to run `t3claw status` for full details
    println!(
        "  {}Run `t3claw status` for full system details.{}",
        fmt::hint(),
        fmt::reset()
    );
