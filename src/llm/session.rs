...
//! OAuth flow. Tokens are stored in `~/.t3claw/session.json` and refreshed
...
pub struct SessionConfig {
    ...
    /// Path to session file (e.g., ~/.t3claw/session.json).
...
impl SessionManager {
    ...
    /// `~/.t3claw/.env` for survival across restarts.
    ...
    async fn api_key_login(&self) -> Result<(), LlmError> {
        ...
        // Persist to ~/.t3claw/.env so the key survives restarts
