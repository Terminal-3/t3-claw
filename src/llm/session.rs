...
//! OAuth flow. Tokens are stored in `~/.bastionclaw/session.json` and refreshed
...
pub struct SessionConfig {
    ...
    /// Path to session file (e.g., ~/.bastionclaw/session.json).
...
impl SessionManager {
    ...
    /// `~/.bastionclaw/.env` for survival across restarts.
    ...
    async fn api_key_login(&self) -> Result<(), LlmError> {
        ...
        // Persist to ~/.bastionclaw/.env so the key survives restarts
