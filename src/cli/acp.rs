...
async fn list_agents() -> anyhow::Result<()> {
    ...
    if agents.agents.is_empty() {
        ...
        println!("  t3claw acp add goose --command goose --arg \"--stdio\"");
        return Ok(());
    }
...
async fn test_agent(name: &str) -> anyhow::Result<()> {
    ...
    /// Event sink that prints agent output to stdout during `t3claw acp test`.
    ...
    // Uses T3ClawAcpClient with a PrintEventSink so the test exercises
    ...
    let result = local_set
        .run_until(async move {
            ...
            let client = acp_bridge::T3ClawAcpClient::new(PrintEventSink);
            ...
            let handshake = tokio::time::timeout(
                ...
                conn.initialize(acp_bridge::t3claw_init_request()),
            )
            .await;
