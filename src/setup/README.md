...
This document is the authoritative specification for BastionClaw's onboarding
...
3. If Command::Run or no command:
   a. Load .env files (dotenvy::dotenv() then load_bastionclaw_env())
...
`DATABASE_BACKEND` live in `~/.bastionclaw/.env`.
...
```
auto_setup_database()    → libsql at ~/.bastionclaw/bastionclaw.db (zero prompts)
...
Step 2/2: Model Selection     ← only interactive step
       ↓
   save_and_summarize()      → includes tip to run `bastionclaw onboard`
```
...
defaults to libsql at `~/.bastionclaw/bastionclaw.db`, creates the database,
...
The full 9-step wizard remains available via `bastionclaw onboard`.
...
**libSQL path:**
1. Offer local path (default: `~/.bastionclaw/bastionclaw.db`)
...
2. "Allow bastionclaw to access this keychain item" (per-app authorization)
...
- **Never probe the keychain in read-only commands** (e.g., `bastionclaw status`).
...
- **NEAR AI Chat** path: session token saved to `~/.bastionclaw/session.json`.
  Hosting providers can set `NEARAI_SESSION_TOKEN` env var directly (takes
  precedence over file-based tokens).
- **NEAR AI Cloud** path: `NEARAI_API_KEY` saved to `~/.bastionclaw/.env`
...
6b. Discover WASM channels from ~/.bastionclaw/channels/
...
1. Already installed in `~/.bastionclaw/channels/`
...
4. Discover already-installed tools in `~/.bastionclaw/tools/`
...
**Layer 1: `~/.bastionclaw/.env`** (bootstrap vars)
...
LIBSQL_PATH="/Users/name/.bastionclaw/bastionclaw.db"
...
DATABASE_URL="postgres://user:pass@localhost/bastionclaw"
SECRETS_MASTER_KEY="..."
ONBOARD_COMPLETED="true"
```
...
1. Writes bootstrap vars to `~/.bastionclaw/.env` via `write_bootstrap_env()`
...
Bootstrap vars written to `~/.bastionclaw/.env` (only true chicken-and-egg vars
...
pub struct Settings {
    ...
    // Advanced (not in wizard, set via `bastionclaw config set`)
...
- Service name: `"bastionclaw"`, account: `"master_key"`
...
1. **NEAR AI Cloud API key (option 4 in auth menu):** Get an API key
   ...
   local listener is needed. The key is saved to `~/.bastionclaw/.env`
   and the encrypted secrets store. Uses the OpenAI-compatible
   ChatCompletions API mode.

2. **Custom callback URL:** Set `BASTIONCLAW_OAUTH_CALLBACK_URL` to a
   publicly accessible URL (e.g., via SSH tunnel or reverse proxy) that
   forwards to port 9876 on the server:
   ```bash
   export BASTIONCLAW_OAUTH_CALLBACK_URL=https://myserver.example.com:9876
   ```
...
6. Run the full shipping checklist:
   ...
   ```
7. Test a fresh onboarding: `rm -rf ~/.bastionclaw && cargo run`
