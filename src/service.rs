//! OS service management for running BastionClaw as a daemon.
//!
//! Generates and manages platform-native service definitions:
//! - **macOS**: launchd plist at `~/Library/LaunchAgents/com.bastionclaw.daemon.plist`
//! - **Linux**: systemd user unit at `~/.config/systemd/user/bastionclaw.service`
//!
//! The installed service runs `bastionclaw run` (the default agent mode) and is
...
const SERVICE_LABEL: &str = "com.bastionclaw.daemon";
const SYSTEMD_UNIT: &str = "bastionclaw.service";
...
fn install_macos() -> Result<()> {
    ...
    let logs_dir = bastionclaw_logs_dir();
    ...
    println!("  Start with: bastionclaw service start");
    Ok(())
}
...
fn install_linux() -> Result<()> {
    ...
    let unit = format!(
        "[Unit]\n\
         Description=BastionClaw daemon\n\
...
fn start() -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        if !plist.exists() {
            bail!("Service not installed. Run `bastionclaw service install` first.");
        }
...
fn bastionclaw_logs_dir() -> PathBuf {
    bastionclaw_base_dir().join("logs")
}
...
mod tests {
    ...
    fn macos_plist_path_has_expected_suffix() {
        ...
        assert!(
            s.ends_with("Library/LaunchAgents/com.bastionclaw.daemon.plist"),
            "unexpected path: {s}"
        );
    }
    ...
    fn linux_unit_path_has_expected_suffix() {
        ...
        let s = path.to_string_lossy();
        assert!(
            s.ends_with(".config/systemd/user/bastionclaw.service"),
            "unexpected path: {s}"
        );
    }

    #[test]
    fn logs_dir_under_bastionclaw() {
        let path = bastionclaw_logs_dir();
        let s = path.to_string_lossy();
        assert!(s.ends_with(".bastionclaw/logs"), "unexpected path: {s}");
    }

    #[test]
    fn macos_plist_sets_cli_enabled_false() {
        let plist = macos_plist_content("/tmp/bastionclaw", "/tmp/stdout.log", "/tmp/stderr.log");
