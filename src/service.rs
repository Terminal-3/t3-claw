//! OS service management for running T3Claw as a daemon.
//!
//! Generates and manages platform-native service definitions:
//! - **macOS**: launchd plist at `~/Library/LaunchAgents/com.t3claw.daemon.plist`
//! - **Linux**: systemd user unit at `~/.config/systemd/user/t3claw.service`
//!
//! The installed service runs `t3claw run` (the default agent mode) and is
...
const SERVICE_LABEL: &str = "com.t3claw.daemon";
const SYSTEMD_UNIT: &str = "t3claw.service";
...
fn install_macos() -> Result<()> {
    ...
    println!("  Start with: t3claw service start");
    Ok(())
}
...
fn start() -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        if !plist.exists() {
            bail!("Service not installed. Run `t3claw service install` first.");
        }
...
fn t3claw_logs_dir() -> PathBuf {
    t3claw_base_dir().join("logs")
}
...
mod tests {
    ...
    fn macos_plist_path_has_expected_suffix() {
        ...
        assert!(
            s.ends_with("Library/LaunchAgents/com.t3claw.daemon.plist"),
            "unexpected path: {s}"
        );
    }
    ...
    fn linux_unit_path_has_expected_suffix() {
        ...
        let s = path.to_string_lossy();
        assert!(
            s.ends_with(".config/systemd/user/t3claw.service"),
            "unexpected path: {s}"
        );
    }

    #[test]
    fn logs_dir_under_t3claw() {
        let path = t3claw_logs_dir();
        let s = path.to_string_lossy();
        assert!(s.ends_with(".t3claw/logs"), "unexpected path: {s}");
    }

    #[test]
    fn macos_plist_sets_cli_enabled_false() {
        let plist = macos_plist_content("/tmp/t3claw", "/tmp/stdout.log", "/tmp/stderr.log");
