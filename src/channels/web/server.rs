...
pub struct GatewayState {
    ...
    pub skill_registry: Option<Arc<std::sync::RwLock<t3claw_skills::SkillRegistry>>>,
...
use t3claw_gateway::{FrontendBundle, LayoutConfig, NONCE_PLACEHOLDER};
...
/// See `crates/t3claw_gateway/static/app.js` — the layout-config IIFE
/// already reads `window.__T3CLAW_LAYOUT__`, which a future change can
...
async fn oauth_callback_handler(
    ...
    let state_param = match params.get("state") {
        ...
        _ => {
            return oauth_error_page("T3Claw");
        }
    };
    ...
    if success {
        axum::response::Html(
            "<html><body style='font-family: system-ui; text-align: center; padding: 60px;'>\
             ...
             <p>You can close this tab and return to T3Claw.</p>\
                ...
                crate::extensions::ExtensionSource::WasmBuildable { .. } => {
                    format!(
                        "'{}' requires building from source. \
                         Run `t3claw registry install {}` from the CLI.",
                        req.name, req.name
                    )
                }
                _ => format!(
...
mod tests {
    ...
    async fn test_oauth_callback_happy_path_with_gateway_token_fallback() {
        ...
        let _exchange_url_guard =
            set_env_var("T3CLAW_OAUTH_EXCHANGE_URL", Some(&proxy.base_url()));
    ...
    /// `~/.t3claw/mcp-servers.json`. Any test that calls `install` for an
