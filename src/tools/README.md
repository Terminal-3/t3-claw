...
5. Install with `bastionclaw tool install path/to/tool.wasm`
...
When running `bastionclaw tool auth <tool>`:
...
Both are first-class in the extension system (`bastionclaw tool install` handles both), but they have different strengths.

**WASM Tools (BastionClaw native)**
...
- Cost: external process with full system access (no sandbox), manages own credentials, BastionClaw can't prevent leaks
