...
5. Install with `t3claw tool install path/to/tool.wasm`
...
When running `t3claw tool auth <tool>`:
...
Both are first-class in the extension system (`t3claw tool install` handles both), but they have different strengths.

**WASM Tools (T3Claw native)**
...
- Cost: external process with full system access (no sandbox), manages own credentials, T3Claw can't prevent leaks
