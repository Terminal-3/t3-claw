    ...
    Fut: Future<Output = Result<String, SkillFetchError>>,
{
    ...
    while let Some(dep_name) = queue.pop_front() {
        ...
        match fetcher(download_url).await {
            Ok(dep_content) => {
                let normalized = t3claw_skills::normalize_line_endings(&dep_content);
                match t3claw_skills::registry::SkillRegistry::prepare_install_to_disk(
                    &user_dir,
                    &dep_name,
                    &normalized,
                )
...
impl Tool for SkillInstallTool {
    ...
    async fn execute(
        ...
        let normalized = t3claw_skills::normalize_line_endings(&content);
        ...
        let (skill_name, loaded_skill) =
            t3claw_skills::registry::SkillRegistry::prepare_install_to_disk(
                &user_dir,
...
pub(crate) async fn fetch_skill_content(url: &str) -> Result<String, SkillFetchError> {
    ...
    // Basic size check
    if content.len() as u64 > t3claw_skills::MAX_PROMPT_FILE_SIZE {
        return Err(SkillFetchError::from_message(format!(
