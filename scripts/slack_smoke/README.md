...
Exercises the real Slack WASM channel integration against a running T3Claw instance using live Slack API calls.

## Prerequisites

- **Python 3.11+**
- **Running T3Claw** instance with the Slack channel configured and activated
- **Slack App** with:
...
1. **User token** (`xoxp-`): Sends messages as a real Slack user, which triggers Slack to send webhook events to T3Claw
...
2. Slack sends webhook event to T3Claw
3. T3Claw processes event and calls `chat.postMessage`
