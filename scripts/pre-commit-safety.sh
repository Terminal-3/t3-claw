...
#
# Also runs check-i18n-parity.sh when crates/bastionclaw_gateway/static/i18n/*.js
...
if git diff --cached --quiet 2>/dev/null; then
    I18N_CHANGED=$(git diff --name-only -- 'crates/bastionclaw_gateway/static/i18n/*.js' 2>/dev/null || true)
else
    I18N_CHANGED=$(git diff --cached --name-only -- 'crates/bastionclaw_gateway/static/i18n/*.js' 2>/dev/null || true)
fi
