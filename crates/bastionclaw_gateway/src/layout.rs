...
pub struct BrandingConfig {
    /// Page title (replaces default "BastionClaw").
    ...
    /// side (`window.__BASTIONCLAW_LAYOUT__` and
...
/// (`window.__BASTIONCLAW_LAYOUT__`) and the response body of
...
impl BrandingConfig {
    ...
    /// `window.__BASTIONCLAW_LAYOUT__` JSON island. The first consumer that
...
mod tests {
    ...
    fn test_branding_serialize_drops_hostile_urls() {
        ...
        // `window.__BASTIONCLAW_LAYOUT__` or `GET /api/frontend/layout`.
        let hostile = BrandingConfig {
