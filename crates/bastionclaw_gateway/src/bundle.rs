...
pub const NONCE_PLACEHOLDER: &str = "__BASTIONCLAW_CSP_NONCE__";
...
/// emits `window.__BASTIONCLAW_LAYOUT__` so the browser-side IIFE can read it
...
///
/// **Before `</body>`:**
/// - Layout config as `window.__BASTIONCLAW_LAYOUT__`
    ...
    match serde_json::to_string(&bundle.layout) {
        Ok(layout_json) => {
            ...
            body_injections.push(format!(
                "<script nonce=\"{NONCE_PLACEHOLDER}\">window.__BASTIONCLAW_LAYOUT__ = {safe_layout};</script>"
            ));
        }
        Err(e) => {
            ...
            // ships without `window.__BASTIONCLAW_LAYOUT__`, and the IIFE in
            ...
            tracing::warn!(
                error = %e,
                "failed to serialize LayoutConfig for window.__BASTIONCLAW_LAYOUT__ injection — \
...
mod tests {
    ...
    const MINIMAL_HTML: &str =
        "<!DOCTYPE html><html><head><title>BastionClaw</title></head><body></body></html>";
    ...
    fn test_assemble_index_no_customizations() {
        ...
        assert!(result.contains("window.__BASTIONCLAW_LAYOUT__"));
    ...
    fn test_assemble_index_branding_title() {
        ...
        assert!(!result.contains("<title>BastionClaw</title>"));
    }
    ...
    fn test_assemble_index_layout_json_script_breakout_escaped() {
        ...
        // of the `window.__BASTIONCLAW_LAYOUT__` script injection.
        let bundle = FrontendBundle {
            layout: LayoutConfig {
                branding: BrandingConfig {
        ...
        // but it's also present inside the JSON string in window.__BASTIONCLAW_LAYOUT__.
        let layout_start = result.find("window.__BASTIONCLAW_LAYOUT__").unwrap();
