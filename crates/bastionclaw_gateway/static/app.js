// BastionClaw Web Gateway - Client
...
function getThemeMode() {
  const stored = localStorage.getItem('bastionclaw-theme');
...
function toggleTheme() {
  ...
  localStorage.setItem('bastionclaw-theme', next);
  applyTheme(next);
}
...
document.getElementById('settings-theme-toggle')?.addEventListener('click', () => {
  ...
  if (btn) {
    const mode = localStorage.getItem('bastionclaw-theme') || 'system';
...
function authenticate() {
  ...
  apiFetch('/api/chat/threads')
    .then(() => {
      sessionStorage.setItem('bastionclaw_token', token);
      initApp();
    })
    .catch(() => {
      sessionStorage.removeItem('bastionclaw_token');
...
async function authenticateWithNear() {
  ...
  try {
    ...
    var signed = await wallet.signMessage({
      ...
      recipient: challenge.recipient || 'bastionclaw',
      nonce: nonceBytes,
    });
...
(function autoAuth() {
  ...
  if (sessionStorage.getItem('bastionclaw_oidc') === '1') {
    oidcProxyAuth = true;
  }
  const saved = sessionStorage.getItem('bastionclaw_token');
  if (saved) {
  ...
  fetch('/api/gateway/status', { credentials: 'include' }).then(function(r) {
    if (r.ok) {
      oidcProxyAuth = true;
      sessionStorage.setItem('bastionclaw_oidc', '1');
      document.getElementById('auth-screen').style.display = 'none';
      document.getElementById('app').style.display = 'flex';
      initApp();
    }
...
function connectSSE(lastEventIdOverride) {
  ...
  // NOT wrapped because the BastionClaw gateway never emits SSE frames without
  // an `event:` field — every frame carries a typed name (`response`,
  // `tool_started`, `gate_required`, etc.). Widget authors should subscribe
  // to those typed events via `BastionClaw.api.on('<event_type>', handler)`
  ...
  eventSource.addEventListener = function(type, listener, opts) {
    _origAddEventListener(type, function(e) {
      ...
      if (BastionClaw.api && e.data) {
        try {
          var parsed = JSON.parse(e.data);
          BastionClaw.api._dispatch(type, parsed);
        } catch (parseErr) {
          console.warn('[BastionClaw] SSE parse error for event', type, parseErr);
        }
      }
...
// extensible chat renderers via BastionClaw.registerChatRenderer().
...
function upgradeStructuredData(contentEl) {
  ...
  var renderers = (window.BastionClaw && BastionClaw._chatRenderers) || [];
  for (var i = 0; i < renderers.length; i++) {
    try {
      ...
      console.error('[BastionClaw] Chat renderer "' + renderers[i].id + '" failed:', e);
    }
  }
  ...
  var layoutCfg = window.__BASTIONCLAW_LAYOUT__;
...
function fetchGatewayStatus() {
  apiFetch('/api/gateway/status').then(function(data) {
    ...
    // Version
    if (data.version) {
      html += '<div class="gw-section-label">BastionClaw v' + escapeHtml(data.version) + '</div>';
...
function loadChannelsStatus() {
  ...
  Promise.all([
    ...
    builtinList.appendChild(renderBuiltinChannelCard(
      ...
      I18n.t('channels.runWith', { cmd: 'bastionclaw run --cli' })
    ));
    ...
    builtinList.appendChild(renderBuiltinChannelCard(
      ...
      I18n.t('channels.runWith', { cmd: 'bastionclaw run --repl' })
    ));
...
document.getElementById('user-logout-btn').addEventListener('click', function() {
  fetch('/auth/logout', { method: 'POST', credentials: 'include' })
    .finally(function() {
      sessionStorage.removeItem('bastionclaw_token');
      sessionStorage.removeItem('bastionclaw_oidc');
...
function exportSettings() {
  apiFetch('/api/settings/export').then(function(data) {
    ...
    a.href = url;
    a.download = 'bastionclaw-settings.json';
...
//
// Widget authors call BastionClaw.registerWidget({ id, name, slot, init, ... })
// from their module script. The init() function receives a container DOM element
// and the BastionClaw.api object for authenticated fetch, event subscription, etc.

// Define `window.BastionClaw` as a non-writable, non-configurable property
// rather than `window.BastionClaw = window.BastionClaw || {}`. The `|| {}` form
// would honor any pre-existing value on `window.BastionClaw`, which in
...
// has no inline scripts that touch `window.BastionClaw`, so this is
...
// `BastionClaw` object after the fact — its only path is to mutate properties
// on the fixed object, which is the same authority every other widget has.
Object.defineProperty(window, 'BastionClaw', {
  value: {},
  writable: false,
  configurable: false,
  enumerable: true,
});
BastionClaw.widgets = new Map();
BastionClaw._widgetInitQueue = [];
BastionClaw._chatRenderers = [];
 ...
 */
BastionClaw.registerWidget = function(def) {
  if (!def.id || !def.init) {
    console.error('[BastionClaw] Widget registration requires id and init:', def);
    return;
  }
  BastionClaw.widgets.set(def.id, def);
 ...
 */
BastionClaw.registerChatRenderer = function(def) {
  if (!def.id || !def.match || !def.render) {
    console.error('[BastionClaw] Chat renderer requires id, match, and render:', def);
    return;
  }
  BastionClaw._chatRenderers.push(def);
  // Sort by priority (higher first)
  BastionClaw._chatRenderers.sort(function(a, b) {
  ...
  fetch: function(path, opts) {
    ...
    try {
      ...
      return Promise.reject(
        new TypeError('BastionClaw.api.fetch: invalid URL ' + JSON.stringify(path))
      );
    }
    if (resolved.origin !== window.location.origin) {
      return Promise.reject(
        new TypeError(
          'BastionClaw.api.fetch: cross-origin requests are not allowed (got ' +
  ...
  _dispatch: function(eventType, data) {
    ...
    for (var i = 0; i < handlers.length; i++) {
      try { handlers[i](data); } catch (e) {
        console.error('[BastionClaw] Widget event handler error (' + eventType + '):', e);
      }
    }
  },
...
function _addWidgetTab(def) {
  ...
  if (!tabBar || !tabContent) {
    ...
    BastionClaw._widgetInitQueue.push(def);
    return;
  }
  ...
  try {
    def.init(panel, BastionClaw.api);
  } catch (e) {
    console.error('[BastionClaw] Widget "' + def.id + '" init failed:', e);
...
if (window.__BASTIONCLAW_LAYOUT__) {
  (function() {
    var layout = window.__BASTIONCLAW_LAYOUT__;
...
if (BastionClaw._widgetInitQueue && BastionClaw._widgetInitQueue.length > 0) {
  BastionClaw._widgetInitQueue.forEach(function(def) {
    _addWidgetTab(def);
  });
  BastionClaw._widgetInitQueue = [];
}
...
if (window.__BASTIONCLAW_LAYOUT__
    && window.__BASTIONCLAW_LAYOUT__.tabs
    && window.__BASTIONCLAW_LAYOUT__.tabs.default_tab
    && !window.location.hash) {
  switchTab(window.__BASTIONCLAW_LAYOUT__.tabs.default_tab);
}
