...
pub const NONCE_PLACEHOLDER: &str = "__T3CLAW_CSP_NONCE__";
...
/// emits `window.__T3CLAW_LAYOUT__` so the browser-side IIFE can read it
...
///
/// **Before `</body>`:**
/// - Layout config as `window.__T3CLAW_LAYOUT__`
    ...
    match serde_json::to_string(&bundle.layout) {
        Ok(layout_json) => {
            ...
            body_injections.push(format!(
                "<script nonce=\"{NONCE_PLACEHOLDER}\">window.__T3CLAW_LAYOUT__ = {safe_layout};</script>"
            ));
        }
        Err(e) => {
            ...
            // ships without `window.__T3CLAW_LAYOUT__`, and the IIFE in
            ...
            tracing::warn!(
                error = %e,
                "failed to serialize LayoutConfig for window.__T3CLAW_LAYOUT__ injection — \
...
mod tests {
    ...
    const MINIMAL_HTML: &str =
        "<!DOCTYPE html><html><head><title>T3Claw</title></head><body></body></html>";
    ...
    fn test_assemble_index_no_customizations() {
        ...
        assert!(result.contains("window.__T3CLAW_LAYOUT__"));
    ...
    fn test_assemble_index_branding_title() {
        ...
        assert!(!result.contains("<title>T3Claw</title>"));
    }
    ...
    fn test_assemble_index_layout_json_script_breakout_escaped() {
        ...
        // of the `window.__T3CLAW_LAYOUT__` script injection.
        let bundle = FrontendBundle {
            layout: LayoutConfig {
                branding: BrandingConfig {
        ...
        // but it's also present inside the JSON string in window.__T3CLAW_LAYOUT__.
        let layout_start = result.find("window.__T3CLAW_LAYOUT__").unwrap();
