...
//!
//! Prompt templates live in `crates/bastionclaw_engine/prompts/` as plain
...
impl PlatformInfo {
    ...
    pub fn to_prompt_section(&self) -> String {
        ...
        lines.push("You are **BastionClaw**, a secure autonomous AI assistant platform.".into());
...
mod tests {
    ...
    async fn prompt_with_platform_info_injects_identity() {
        let info = PlatformInfo {
            ...
            repo_url: Some("https://github.com/nearai/bastionclaw".into()),
        };
        let prompt =
            build_codeact_system_prompt(&[], None, ProjectId(uuid::Uuid::nil()), Some(&info)).await;
        assert!(prompt.contains("BastionClaw"));
        ...
        assert!(prompt.contains("github.com/nearai/bastionclaw"));
    }
