---
description: Deep audit of the T3Claw crate for vulnerabilities, bugs, unfinished work, inconsistencies, and oversights
...
T3Claw convention: `.unwrap()` and `.expect()` are banned in production code. Any occurrence outside `#[cfg(test)]` blocks is a **High severity** finding.
...
T3Claw has two database backends (PostgreSQL and libSQL). Check both for injection vectors.
...
- Is `thiserror` used consistently for error types (T3Claw convention)?
...
T3Claw convention: use `crate::` imports, not `super::`. Flag any `super::` imports in non-test code.
...
T3Claw key traits: `Database` (~60 methods), `Channel`, `Tool`, `LlmProvider`, `SuccessEvaluator`, `EmbeddingProvider`. If any new methods were added to `Database`, verify both `postgres.rs` and `libsql_backend.rs` implement them.
